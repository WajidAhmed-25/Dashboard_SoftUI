...
class FollowUp extends Component {
  constructor(props) {
    ...
    this.fetchDataUrl = `${apiUrl}/api/email_response/get/follow_up?page=`;
  ...
  fetchData(page) {
    axios
      ...
      .then((response) => {
        console.log("API response FollowUp:", response.data.data);
  ...
  handleFilterChange(event) {
    ...
    if (filterValue === "") {
      ...
      const filteredRows = this.state.rows.filter(
        (row) => row.order_type === filterValue || row.required_file_format === filterValue
      );
          ...
          <SoftBox mb={3}>
            ...
            <Card>
              <SoftBox display="flex" justifyContent="space-between" alignItems="center" p={3}>
                <SoftTypography variant="h6">Follow Up</SoftTypography>
              </SoftBox>
              <SoftBox>
