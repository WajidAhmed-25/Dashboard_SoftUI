...
class ClientReportingPage extends Component {
  constructor(props) {
    super(props);
    this.state = {
      rows: [],
    ...
    this.fetchDataUrl = "http://127.0.0.1:8000/api/email-responses/all";
    ...
    axios
      ...
      .then((response) => {
        if (response.data.success) {
          ...
          this.setState({ rows: response.data.data, filteredRows: response.data.data }); // initialize filteredRows with all data
        } else {
  ...
  render() {
    ...
    const columns = [
      ...
      { name: "Email Id", accessor: "email_id" },
      { name: "LODate", accessor: "lodate" },
    ];

    // Map fetched data to table rows
    const tableRows = currentItems.map((data) => ({
      client_code: data.client_code || "N/A",
      email_id: data.email_address,
      lodate: data.date,
    }));
          ...
          <SoftBox mb={3}>
            ...
            <Card>
              <SoftBox display="flex" justifyContent="space-between" alignItems="center" p={3}>
                <SoftTypography variant="h6">Client Reporting</SoftTypography>
              </SoftBox>
              <SoftBox
                sx={{
...
export default ClientReportingPage;
