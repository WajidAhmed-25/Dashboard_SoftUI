...
class Logs extends Component {
  constructor(props) {
    ...
    this.fetchDataUrl = `${apiUrl}/api/order-logs?page=`;
          ...
          const transformedRows = response.data.map((item) => ({
            ...
            edit: (
              <SoftTypography
                ...
                onClick={(e) => {
                  e.preventDefault();
                  this.handleEdit(item.order_id); // Changed from item.id to item.order_id
                }}
                style={{ cursor: "pointer" }}
              >
                Edit
  ...
  handleView(order_id) {
    // Changed from id to order_id in the route
    window.location.href = `/logs/view/${order_id}`;
    // Or if using React Router:
    // this.props.history.push(`/logs/view/${order_id}`);
  }
