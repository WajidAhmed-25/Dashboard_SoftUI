...
class Account extends Component {
  ...
  fetchData() {
    axios
      ...
      .then((response) => {
        ...
        // Mapping the response to extract necessary fields
        const mappedRows = response.data.map((item) => ({
          id: item.id, // id from the main object
          order_no: item.email_response.order_id, // order_id from email_response
          client_id: item.email_response.client_code, // Example of other fields you may want to show
          attachments: item.email_response.attachments,
          pricing: item.email_response.price,
        }));
  
        this.setState({ rows: mappedRows, filteredRows: mappedRows }); // initialize filteredRows with mapped data
      })
      .catch((error) => {
  ...
  render() {
    const { currentPage, itemsPerPage, filteredRows } = this.state;
    ...
    const columns = [
      { name: "Order No.", accessor: "id" }, // The order ID
      { name: "Client ID", accessor: "client_id" }, // Client code from email_response
      { name: "Attachments", accessor: "attachments" }, 
      { name: "Pricing", accessor: "pricing" },
    ];
